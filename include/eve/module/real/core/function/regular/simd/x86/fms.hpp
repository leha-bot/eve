...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE
  wide<T, N> fms_(EVE_SUPPORTS(avx512_)
  ...
  requires(is_one_of<D>(types<toward_zero_type, downward_type, to_nearest_type, upward_type> {}))
  {
    constexpr auto c = categorize<wide<T, N>>();
          ...
          if constexpr(c == category::float32x16) return _mm512_fmsub_round_ps(v,w,x,D::base_type::value);
    else  if constexpr(c == category::float64x8 ) return _mm512_fmsub_round_pd(v,w,x,D::base_type::value);
    else return fms_(EVE_RETARGET(cpu_), D(), v, w, x);
  ...
  EVE_FORCEINLINE
  wide<T, N> fms_(EVE_SUPPORTS(avx512_), C const &cx
                , D const &, wide<T, N> const &v, wide<T, N> const &w, wide<T, N> const &x) noexcept
  requires(is_one_of<D>(types<toward_zero_type, downward_type, to_nearest_type, upward_type> {}))
  {
    constexpr auto c = categorize<wide<T, N>>();
    ...
    if constexpr( C::is_complete || abi_t<T, N>::is_wide_logical )
    {
      return fms_(EVE_RETARGET(cpu_),cx,D(),v,w,x);
    }
    else
    {
      auto src  = alternative(cx,v,as<wide<T, N>>{});
      auto m    = expand_mask(cx,as<wide<T, N>>{}).storage().value;
            ...
            if constexpr(c == category::float32x16) return _mm512_mask_fmsub_round_ps(v,m,w,x,D::base_type::value);
      else  if constexpr(c == category::float64x8 ) return _mm512_mask_fmsub_round_pd(v,m,w,x,D::base_type::value);
      else return fms_(EVE_RETARGET(cpu_), cx, D(), v, w, x);
    }
 }
}
