...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE
  wide<T, N> add_(EVE_SUPPORTS(avx512_)
  ...
  EVE_FORCEINLINE
  wide<T, N> add_(EVE_SUPPORTS(avx512_), C const &cx
                , D const &, wide<T, N> const &v, wide<T, N> const &w) noexcept
  requires(is_one_of<D>(types<toward_zero_type, downward_type, to_nearest_type, upward_type> {}))
  {
    constexpr auto c = categorize<wide<T, N>>();
    ...
    if constexpr( C::is_complete || abi_t<T, N>::is_wide_logical )
    {
      return add_(EVE_RETARGET(cpu_),cx,D(),v,w);
    }
    else
    {
      auto src  = alternative(cx,v,as<wide<T, N>>{});
      auto m    = expand_mask(cx,as<wide<T, N>>{}).storage().value;
            ...
            if constexpr(c == category::float32x16) return _mm512_mask_add_round_ps(src,m,v,w,D::base_type::value);
      else  if constexpr(c == category::float64x8 ) return _mm512_mask_add_round_pd(src,m,v,w,D::base_type::value);
      else return add_(EVE_RETARGET(cpu_), cx, D(), v, w);
    }
 }
}
