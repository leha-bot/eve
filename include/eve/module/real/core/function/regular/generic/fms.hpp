...
namespace eve::detail
{
  ...
  EVE_FORCEINLINE auto fms_(EVE_SUPPORTS(cpu_), C const &cond, T const &a, U const &b, V const &c) noexcept
  requires properly_convertible<U, V, T>
  {
    using r_t =  common_compatible_t<T, U, V>;
  ...
  EVE_FORCEINLINE T fms_(EVE_SUPPORTS(cpu_), D const &, T a, T b, T c) noexcept
  requires  has_native_abi_v<T>
  && (is_one_of<D>(types<toward_zero_type, downward_type, to_nearest_type, upward_type> {}))
  {
    return D()(round)(fms(a, b, c));
  }
  ...
  EVE_FORCEINLINE T fms_(EVE_SUPPORTS(cpu_), C const &cond, D const &, T a, T b, T c) noexcept
  requires  has_native_abi_v<T>
  && (is_one_of<D>(types<toward_zero_type, downward_type, to_nearest_type, upward_type> {}))
  {
    auto tmp = mask_op( cond, eve::fms, a, b, c);
