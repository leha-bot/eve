...
namespace eve::detail
{
...

  {
    ...
    else
    {
      ...
      advance(curn, 1);
      auto dfma = d(fma);
      r_t that(dfma(sub(x,  r_t(*firstn)),  r_t(*firstc),  r_t(*curc)));
      ...
      for (advance(curc, 1); curc != lastc; advance(curc, 1), advance(curn, 1))
        that = step(that,  r_t(*curc),  r_t(*curn));
      return that;
    }
  }
